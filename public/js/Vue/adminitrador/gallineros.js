function init()
{
	...
	var Inactivos='/GallinerosInactivos';
...
new Vue({
	...
	methods:{
		...
		getGallinerosInactivos:function(){
			this.$http.get(route+Inactivos).then
			(function(response){
				console.log(response);
				this.gallinerosInactivos=response.data;
			});
		},
...
computed:{
		...
		filtroGallineros2:function(){
			return this.gallinerosInactivos.filter((gallinero)=>{
				return gallinero.nombre.toLowerCase().match(this.search.toLowerCase().trim());
			});
		}
	}
